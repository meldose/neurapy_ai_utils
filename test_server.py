...
class Mairakinematics(Node):
    ...
    def execute_callback(self, goal_handle) -> FollowJointTrajectory.Result:
        ...
        for idx, pt in enumerate(traj.points):
            ...
            self._publish_command(pt.positions, pt.time_from_start)
    ...
    def _publish_command(self, positions: List[float], time_from_start) -> None:
    ...
    def _throw_if_list_poses_invalid(self, poses: List[List[float]]) -> None:
        ...
        for p in poses:
            self._throw_if_pose_invalid(p)

    def _get_ik_solution(
        self, goal_pose_cartesian: List[float], reference_joint_states: List[float]
    ) -> List[float]:
        """Random-seed IK solver retry up to 100 times for valid solution."""
    ...
    def cartesian_2_joint(
        self, goal_pose_cartesian: List[float], reference_joint_states: Optional[List[float]]=None
    ) -> List[float]:
    ...
    def set_motion_till_force(
        self, stopping_forces=[0,0,1], reflex_mode_after_contact=True
    ) -> None:
    ...
    def _is_id_successful(self, id: int, timeout=10.0) -> Tuple[bool,bool]:
    ...
    def _execute_if_successful(self, id: int) -> bool:
        ...
        plan_ok, exec_ok = self._is_id_successful(id)
